...
def main():
    ...
    if err is not None:
        return
    meld_na_scores = np.reshape(meld_na_scores, (num_obs, 1))
    time_stamps = np.reshape(time_stamps, (num_obs, 1))

    print(f"meld_na_scores {meld_na_scores.shape}")
    ...
    meld_na_scores = meld_scaler.transform(meld_na_scores)
    time_stamps = timestamp_scaler.transform(time_stamps)

    data = np.concatenate((
        np.reshape(meld_na_scores, (1, -1)),
        np.reshape(time_stamps, (1, -1))), axis=1)
